...
use deadpool_redis::{Connection, redis::PubSub};
...
pub async fn insert(
    ...
    let queue_key: String = get_message_queue_key(user_id.clone(), device_id);
    let queue_metadata_key: String = get_message_queue_metadata_key(user_id.clone(), device_id);
    let queue_index_key: String = get_queue_index_key(user_id.clone(), device_id);
...
pub async fn listen_for_expirations(mut conn: redis::Connection) {
    ...
    while let Ok(msg) = con.get_message() {
        ...
        println!("{}", key);
        send_message_to_db(key);
    }
}

pub fn send_message_to_db(msg_id_key: String) {
    let user_msg_id = format!("user_messages::{}}}", msg_id_key.strip_prefix("msg_key::").unwrap_or(&*msg_id_key) .split('<').next().unwrap_or("").trim_end_matches('}'));
    println!("{}",user_msg_id);
